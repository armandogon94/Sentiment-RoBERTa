...
*Generated by `evaluate.py`. Every number below was read from a `runs/*/metrics.json` produced by a command on the owner's machine — nothing here is estimated, rounded up, carried over from the source notebook, or taken from a paper.*
...
- **Reproduce** — `uv run python train.py -c cfg/small.yaml` then `uv run python evaluate.py -i runs/latest -o reports/RESULTS.md`

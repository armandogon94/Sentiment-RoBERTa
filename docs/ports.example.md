# Port allocation: Sentiment-RoBERTa
...
This is an ML-research repo: a config-driven training entrypoint that writes to `runs/run_N/` and a
markdown report. There is no service to run. The ports above are **reserved so that nothing in this
repo can ever collide with another project on the same machine**, not because a server exists.
Reserving a port and binding nothing is the correct outcome here, so do not invent a dashboard to
justify a number in this table.
...
1. Never bind a framework default: `5432`, `6379`, `3000`, `8000`, `5000`, `7000`.
   On macOS, `5000` and `7000` are taken by the AirPlay Receiver.

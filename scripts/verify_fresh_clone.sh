...
# Every 0.xxxx in the README must appear in a COMMITTED measured artifact. runs/ is gitignored,
# so the committed evidence chain is: reports/RESULTS.md (generated by evaluate.py from
# runs/*/metrics.json), docs/PROGRESS.md (the measured-numbers table), and the re-run notebook's
# saved cell outputs — which are themselves a measured artifact, and the reason Slice 7 exists.
orphans=0
while read -r n; do
  grep -qr -- "$n" reports/ docs/PROGRESS.md notebooks/ 2>/dev/null \
    || { echo "    ORPHAN: $n"; orphans=1; }
